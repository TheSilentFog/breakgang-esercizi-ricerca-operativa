...
minimize z1: (sum {p in Pazienti, t in Triage, o in Ospedali} (tempo[t,o]/tmin[t,p]) * x[p,t,o]) / (sum {p in Pazienti} tot[p]);

# Minimizzare il massivo sovraccarica tra gli ospedali
var z2;
subject to MinMax {o in Ospedali}: (sum {t in Triage, p in Pazienti} x[p,t,o]) / (sum {t in Triage,p in Pazienti} distr[t,p] / nO) <= z2;

# Metodo dei vincoli: analisi parametrica
param delta := 1.467;
s.t. Parametrica: z2 <= delta;
...
# Risultato dell'analisi parametrica: punti di discontinuità
     z2        z1
   1.12222   1.23225
   1.21111   1.22058
   1.28889   1.21037
