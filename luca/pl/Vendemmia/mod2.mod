...
var produzione{vini} >= 0 integer;                  # cartoni

param litri_cartone := 12 * 0.75;                                # litri / cartone



s.t. vincolo_produzione{u in uve}:
    sum{v in vini} produzione[v] * litri_cartone * coefficenti_vinifici[u,v]
    <= uva_raccolta[u] * uvificazione;

s.t. vincolo_produzione_perc_low{v in vini}:
    produzione[v] * litri_cartone >= (sum{vin in vini} produzione[vin] * litri_cartone) * min_prod[v];

s.t. vincolo_produzione_perc_upp{v in vini}:
    produzione[v] * litri_cartone <= (sum{vin in vini} produzione[vin] * litri_cartone) * max_prod[v];



maximize alcolismo:
    sum{v in vini} produzione[v] * litri_cartone * valore_vino[v];
